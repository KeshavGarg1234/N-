...
export function LiveParticipants({ 
  ...
  if (!canSeeReactions && !isExpandedView) {
    return (
        <div className="relative w-full rounded-xl overflow-hidden border border-white/10 bg-zinc-950/40 min-h-[140px] flex flex-col items-center justify-center p-6 text-center shadow-inner">
            ...
            <h3 className="text-base font-bold text-white mb-1">
                {isLiveDisabled ? "Live Reactions Blocked" : "Join the Session"}
            </h3>
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      for (const change of snapshot.docChanges()) {
        if (change.type === 'added') {
          ...
          if (data.timestamp < mountTime.current - 1000) continue;
